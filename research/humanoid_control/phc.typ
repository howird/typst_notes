...
- *Challenge: Catastrophic Forgetting in Large-Scale Motion Imitation*
  - *Problem:* When a single reinforcement learning policy is trained on a vast
    and diverse motion dataset (like AMASS, with over 10,000 clips), it tends to
    "forget" how to perform previously learned motions as it learns new, more
    difficult ones.
  - *Hypothesis:* By progressively allocating new, dedicated network capacity to
    ...
    existing skills.
  - *Approach:* The paper proposes the *Progressive Multiplicative Control
    Policy (PMCP)*. This method first trains a baseline "primitive" network on
    the entire dataset. It then identifies the motion sequences that this
    primitive fails to imitate "hard negative mining" and trains a new, separate
    primitive specifically on this harder subset. This process can be repeated,
    creating a stack of specialized primitives. Finally, a "composer" network is
    trained to dynamically combine the outputs of these frozen primitives to
    control the humanoid.
  - *Alternative Solutions:*
    - *Residual Force Control (RFC):* Using external, non-physical forces to
      stabilize the character. This is effective but can lead to unrealistic
      artifacts like floating or flying.
    - *Mixture of Experts (MoE):* Using multiple expert policies, though these
      have not yet been scaled successfully to the largest motion datasets.
...
- *Challenge: Handling Noisy Inputs and Recovering from Falls*
  - *Problem:* Controllers often fail when given noisy inputs, such as pose
    estimations from a live video feed, causing the humanoid to fall.
    Traditional methods simply reset the simulation, which is disruptive and
    ineffective if the reference pose is also unreliable.
  - *Hypothesis:* A dedicated recovery skill, learned as a separate task, can
    allow the humanoid to get up and return to the reference motion in a
    natural, human-like way, creating a more robust and seamless experience.
  - *Approach:* Fail-state recovery is treated as another task for a specialized
    primitive ($P^(F)$). When the humanoid falls or is too far from the
    reference, this primitive takes over. It uses a simplified point-goal
    objective, where the goal is just the root position of the reference motion,
    ignoring the full-body pose. This allows the humanoid to get up and walk
    back to the target location before resuming detailed motion imitation. The
    *Adversarial Motion Prior (AMP)* reward is used to ensure the recovery
    motions appear natural.
  - *Alternative Solutions:*
    - *Resetting:* Teleporting the character to a reference pose upon failure,
      which can be jarring and lead to a vicious cycle of falling and resetting.
    - *Floating-base models:* Using a humanoid model that cannot fall, which
      compromises physical realism.
    - *Non-tracking recovery policies:* Using a recovery policy that does not
      have access to the reference motion, which can result in unnatural,
      jittery movements.

- *Challenge: Requiring Full Rotational Pose Data*
  - *Problem:* Most imitation methods require both 3D joint positions and joint
    rotations as input. However, estimating accurate joint rotations from images
    is a more difficult task than estimating 3D keypoint positions.
  ...
  - *Approach:* The paper develops and evaluates a "keypoint-based" version of
    the controller. This variant takes only 3D keypoint locations
    ($hat(p)_( 1:T )$) as input and learns to generate the necessary joint
    torques to match those target positions, effectively performing a
    physics-aware form of inverse kinematics.
...
- *High-Level Description:* The PHC is a goal-conditioned policy that learns to
  ...
  frozen, pre-trained "primitive" networks and a "composer" network that learns
  to weight their outputs.
- *Inputs:*
  - *Current State ($s_t$):* The character's proprioceptive information (e.g.,
    ...
    simulation).
  - *Goal State ($s_t^g$):* The target reference motion for the next timestep.
    This can be in the form of:
    - Full pose data (joint rotations $hat(theta)_( t+1 )$ and positions
      $hat(p)_( t+1 )$).
    - Keypoint-only data (3D joint positions $hat(p)_( t+1 )$).
- *Output:*
  - *Action ($a_t$):* A set of target joint angles that are fed into a
    Proportional-Derivative (PD) controller, which then computes the joint
    torques to be applied in the physics simulation.

=== Non-Novel Dependencies

- *Environments/Simulators:*
  - *Isaac Gym:* A GPU-based physics simulation environment used for all
...
- *Datasets:*
  ...
  - *Human3.6M (H36M):* A dataset used for evaluating the controller on unseen
    MoCap and on noisy inputs from video.
- *Pre-trained Models (for evaluation/demos):*
  ...
  - *Motion Diffusion Model (MDM):* A text-to-motion generative model used to
    create reference clips from language prompts.
...
- For real-time video-based avatar control, the system assumes the camera is
  stationary, level with the ground, and contains no pitch or roll. Camera
  height must be manually adjusted at the start of a session.
...
motion. This is formalized as a Markov Decision Process (MDP),
...
- *State Space ($cal(S)$)*: The state $s_t$ at any time $t$ is composed of the
  ...
  - *Proprioceptive State ($s_t^p$)*: Contains the character's physical
    properties, such as its 3D body pose ($q_t$), velocity ($dot(q)_t$), and
    optionally its body shape parameters ($beta$).
  - *Goal State ($s_t^g$)*: Encodes the reference motion target. This changes
    based on the controller type and situation.
    - For *rotation-based imitation*, the goal $s_t^"g-rot"$ includes the
      difference between the target and current pose, velocity, and angular
      velocity, as well as the absolute target pose.
    - For *keypoint-based imitation*, the goal $s_t^"g-kp"$ is simpler,
      containing only the difference in position and velocity, and the absolute
      target position.
    - During *fail-state recovery*, the goal switches based on distance, as
...
- *Reward Function ($cal(R)$)*: The total reward $r_t$ guides the learning
  process and is a weighted sum of three distinct components, defined by
  *Equation (1)*:
...
- *Style Reward ($r_t^"amp"$)*: An adversarial reward from the *Adversarial
  Motion Prior (AMP)* framework. A discriminator network is trained to
  distinguish real human motion from the character's simulated motion, pushing
  the policy to produce more natural and human-like behavior.
- *Energy Penalty ($r_t^"energy"$)*: A penalty on the product of joint torque
  and angular velocity, which discourages jerky, high-frequency motions.

- *Policy ($pi$)*: The final controller, *PHC*, uses a *Progressive
  Multiplicative Control Policy (PMCP)* architecture. This policy is a
  combination of multiple "primitive" networks, each specializing in a different
  task or difficulty level, and a "composer" network that dynamically weights
  their outputs. The final action is sampled from a Gaussian distribution whose
  parameters are a weighted product of the distributions from all primitives,
...
- *Inputs (for iteration $k$):*
  ...
  - *Hard Motion Subset $hat(Q)_"hard"^(k)$:* A collection of motion clips that
    the previous primitive, $cal(P)^(k-1)$, failed to imitate. For the first
    iteration, this is the entire dataset $hat(Q)$.
  - *Value Network $V$ and Discriminator $cal(D)$:* Continuously fine-tuned
    networks from the previous iteration.

- *Process:*
  1. *Train Primitive:* The primitive $cal(P)^k$ is trained using PPO on the
    motion subset $hat(Q)_"hard"^k$ until its performance on this subset
    converges.
  2. *Reward Calculation:* During training rollouts, the reward for each step is
    calculated using *Equation (1)*, with the task reward being the imitation
    term $r_t^"g-imitation"$ from *Equation (2)*.
  3. *Hard Negative Mining:* Once $cal(P)^k$ is converged, it's evaluated on its
    training data $hat(Q)_"hard"^k$. All sequences it still fails on are
    collected to form the input for the next iteration, $hat(Q)_"hard"^(k+1)$.
  4. *Freeze Weights:* The weights of the converged primitive $cal(P)^k$ are
    frozen. It will no longer be trained.

- *Outputs:*
  - A set of $K$ frozen, expert imitation primitives
    $cal(P)^(1), ..., cal(P)^(K)$.
...
- *Inputs:*
  ...
  - *Locomotion Dataset $Q^"loco"$:* A hand-picked subset of AMASS containing
    simple walking and running motions.
  - The fine-tuned Value Network $V$ and Discriminator $cal(D)$.

- *Process:*
  1. *Train Primitive:* $cal(P)^(F)$ is trained using PPO on the $Q^"loco"$
    dataset.
  ...
  3. *Reward Calculation:* The reward is calculated using *Equation (4)*, which
    prioritizes reaching the goal destination ($r_t^"g-point"$) while
    maintaining a natural gait via the AMP reward.
...
- *Inputs:*
  - *Frozen Primitives $cal(P)^(1), ..., cal(P)^(K), cal(P)^(F)$:* The
    complete set of expert networks.
...
- *Process:*
  1. *Train Composer:* With all primitives frozen, only the composer network
    $cal(C)$ is trained using PPO on the entire dataset $hat(Q)$.
  2. *Action Generation:* During training, for a given state $s_t$, the composer
    outputs weights $w_t$. The final action is determined by combining the
    outputs of all primitives according to these weights, using the formulation
    in *Equation (6)*.
  3. *Dynamic Reward:* The reward function dynamically switches between the
    imitation reward (*Equation (2)*) and the recovery reward (*Equation (4)*)
    based on the character's distance to the reference motion, as governed by
    *Equation (3)*.

- *Output:*
  - The fully trained *Perpetual Humanoid Controller (PHC)*, comprising the
    frozen primitives and the trained composer.
...
- *Success Rate (Succ)*: The percentage of test sequences completed without the
  humanoid's joints deviating more than 0.5m on average from the reference
  motion. This measures the controller's stability and tracking ability.
...
- *Physics-based Error*: The difference in velocity ($E_"vel"$) and acceleration
  ($E_"acc"$) between the simulation and the reference motion, which indicates
  physical realism and smoothness.

==== 📈 Results and Significance
- *PHC surpasses the SOTA while being physically pure.* The proposed controller
  (`Ours`) achieved a *98.9% success rate* on the AMASS training set and *96.4%*
  on the AMASS test set. This was higher than UHC *with* its external force
  assistance (97.0% and 96.4% respectively) and drastically better than UHC when
  its assistance was removed (84.5% and 62.6%).
- *The motion is smoother and more realistic.* PHC had a significantly lower
  acceleration error ($E_"acc"$) compared to UHC without RFC, indicating it
  avoids the high-frequency jittery movements that the baseline resorts to for
  balance.
- *The keypoint-only controller is a strong alternative.* The `Ours-kp` variant
  performed on par with the full-pose version, validating that high-quality
  motion imitation can be achieved using only 3D keypoint data, which is often
  easier to acquire from video.
...
- *Task*: Imitate motion sequences generated by running video pose estimators on
  the H36M dataset. These estimators, HybrIK and MeTRAbs, are per-frame models
  and produce noisy output, especially in the depth axis.
- *Input Types*:
...
- *Baselines*: The same UHC and PHC variants from the first experiment were
  used.
...
- *Significance*: This is a critical result for practical applications. It
  ...
  simulated avatar directly from a webcam in real-time, successfully bridging
  the gap between noisy computer vision output and stable physics-based
  animation.

==== ⚠️ Limitations
- There is a noticeable degradation in performance in the live demo compared to
  offline tests.
- This is due to the inherent challenges of real-time processing, including
  noisy depth estimates, difficulty in estimating velocity from jittery inputs,
  and fluctuating framerates.
- The real-time system assumes a stationary, level camera, which must be
  manually configured.
...
- *Models*: Both the rotation-based and keypoint-based PHC models were
  evaluated.

==== 📊 Metrics Used
- *Recovery Success Rate (Succ-5s, Succ-10s)*: The percentage of 1000 random
  trials where the controller successfully got up (if fallen) and moved to
  within 0.5m of the reference motion to resume tracking.

==== 📈 Results and Significance
- *Recovery is extremely reliable.* Both PHC models recovered with a very high
  success rate (*>92%* within 10 seconds) across all scenarios, including the
  most difficult combined "Fallen + Far" state.
...
==== ⚠️ Limitations
- While functionally effective, the *naturalness* of the recovery motion could
  be improved.
...
- *Task*: An ablation study was performed by systematically removing key
  components from the full PHC model and measuring the drop in performance on
  the noisy video input task.
- *Ablated Components*:
  - *Relaxed Early Termination (RET)*: The rule change allowing more foot
    freedom for balance.
  ...
  - *Fail-State Recovery Task*: The inclusion of the dedicated recovery
    primitive.
...
- *Progressive training is the most critical component.* The single biggest
  ...
  the network bigger.
- *New skills can be added without catastrophic forgetting.* Adding the
  fail-state recovery task *after* the imitation primitives were trained did not
  degrade imitation performance. This validates that the PMCP framework
  successfully isolates tasks and prevents the learning of a new skill from
  erasing an old one.
