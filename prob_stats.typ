...
#show: dvdtyp.with(
  title: "Probablility and Statistics Notes", subtitle: [], author: "Howard Nguyen-Huu",
)
...
#problem[
  ...
  case that less certain distributions have samples, x, with
  $p(x) arrow 0^+$ which causes $ln p(x) arrow infinity$?
...
- maximize the llh fn $cal(L)(theta; "Data")$ to find a point estimate $hat(theta)$ which
  is MLE, this bives model of best fit
  - sometimes we find this by sampling distribution exactly (z-stat or t-stat with
    samples from a gaussian pop)
...
- that is under the bayesian approach, both
  ...
  - we use sample update our prob distrib used to describe subjective belief in the
    parameter

- bayesian inference leads to a distributional estimate of $theta$ which means
  that both
  - location of params and
  - uncertainty about params are contained in that distributional estimate, this is
    called posterior distribution ($theta | "Data"$)
...
- we can also apply bayes theorem in the inference situation, where our goal is to
  estimate the posteriror distr for the params _given_ the data characterized by
...
- for inference we might want a point estimate of $theta$, we could use posterior
  {maximum/median/mean/mode}
...
=== Example
- our experiment can be represented as $X_i ~ "Bin"(theta, n)$, then likelihood is
  then:
$
...
- priors are a sistributional representation of our belief about the params begore
  we have seen the data
  - we then update these priors with information from data via the likelihood
    function
  - the prior can represent expert knowledge about the params expressed in the form
    of suiable statistical distribution
...
- in some cases little or no prior knwledge may be available and we can use:
  - non-informative priors (eg jeffreys): $pi(theta) alpha sqrt(|cal(I)(theta)|)$ I
    is fisher info
  - vague priors (e.g. gaussian with large variance)
  - conjugate prior if the posterior and the prior belong to the same family of
    distributions. these types of priors are only available to distributions within
    the regular exponential family
- prior is improper if $integral_(-infinity)^(infinity) p(theta)dif theta = infinity$
...
- mathematically even the simplest posterior can be very difficult to deal with,
  this is due (in the first instance) to the problem of integration to obtain the
  normalization constant
- we could consider the posterior only up to proportionality, however integrating
  to find the posterior mean will still be problematic
- even if we can find the maximum posterior estimate via differentiation, we still
  need to integrate to find confidence intervals
...
- *solution*: if we can sample from the posterior distribution, we could use the
  ...
  provide an approximation to the posterior, then we can estimate things like the
  posterior mean
- this is known as monte carlo integration
- summary we can us MC integration if we are able to sample from the posterior
- now we need a general method to sample from any distribution: Monte Carlo Method
  and Markov chains, which we will discuss individualy first
...
- we can use the simulation technique of monte carlo integration to gain an
  ...
  - the method is based upon drawing obervations from the distribution of the
    variable of interest and simply calculating whatever stat we are interested in
  - methods such as rejection and importance sampling can be used to generate
    independent random samples from any ddistribution, however such methods can be
    ineffiicient in higer dimensions

- MCMC is an example of the MC method which we implement through use of markov
  chains
  - mcmc ddoes not produce independent samples from the target density, $pi(x)$, but
    dependent samples
...
- the is what we generate the new state of the chain from a density dependent only
  only upon $x^t$:
...
- in general, we will be interested in continuous state-spaces since our random
  variables of interest will be parameteers in a model, which are usually but not
  always, continuous
...
- the markov chains that we consider for mcmc are constructed in such a way that:
  + they have a stationary distribution
    - that is there exists a p distribution $pi(x)$ such that $X^t ~ pi(x) => X^(t+1) ~ pi(x)$
  + kernel $cal(K)(x^t, x)$ has property: irreducibility
    - for whatever $X^0$ the sequence ${X^t}$ has a positive probability of reaching
      any region of the state-space
  + thay are positive recurrent
    - a chain run for infinite tiem will return to any states an infinite number of
      times
  + they are aperiodoic
    - a state $i$ has period $k$ if any return to state $i$ must occurs in multiples
      of $k$ time steps. $k=1$ for all states implies that the chain is aperiodic
...
- discussion:
  - the usual approach to Markov chain theory is to start with some transition
    kernel, determine conditions uder which there exists an invariant or stationary
    distribution, and then identify the form of that limiting distribution
...
$
- however we don't know the posterior $pi(x)$ -- its the integrating constant, but
  we do know the above, $pi(theta)$ is $alpha f(x|theta) p(theta)$
...
- the algorithm gives a very genereal way to construct MCMC chains that have a
  ...
  - the distribution of interest is know as the target distribution denoted by $pi(x)$
...
+ given the current position, $x^t$, generate a candidate value, $y$ via proposal
  density $q(y|x^t)$
...
- in theory, we have a great deal of flexibility in our choice of proposal
  ...
  - a lot of the owrk we will do is finding out which proposal types that are better
    than others
...
- if $q$ is chosen poorly, then the number of rejections can be high, so that the
  efficiency of the procedure can be low, any will work, bad ones will just take
  very long
...
- note that since the proosal for a random walk is symmetric, we have: $q(y|x)=q(x|y)$
- thus
$
...
- in multivariate distrs we can only look at the proposal plot to see if something
  is good instead of seeing how well the histogram matches the target distribution
...
- recall: MH algo is a rule for moving around a space such that you can use any
  target density as your posterior, then the stationary distribution is the target
  distribution
